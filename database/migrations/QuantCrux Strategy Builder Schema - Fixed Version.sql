/*
# QuantCrux Strategy Builder Schema - Fixed Version
...
-- Create strategy status enum using VARCHAR with constraints
-- Strategies table
...
CREATE TABLE IF NOT EXISTS signal_matched_rules (
    ...
    matched_rules VARCHAR(255) NOT NULL,
    PRIMARY KEY (signal_id, matched_rules)
);
...
CREATE INDEX IF NOT EXISTS idx_strategy_tags_tags ON strategy_tags(tags);
CREATE INDEX IF NOT EXISTS idx_strategy_versions_strategy_id ON strategy_versions(strategy_id);
CREATE INDEX IF NOT EXISTS idx_strategy_indicators_symbol_timeframe ON strategy_indicators(symbol, timeframe);
...
INSERT INTO strategy_tags (strategy_id, tags)
