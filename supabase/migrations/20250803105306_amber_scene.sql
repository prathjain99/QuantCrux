/*
# QuantCrux User Management Schema - Fixed Version
...
-- Users table (using VARCHAR for enum compatibility)
CREATE TABLE users (
    ...
    CONSTRAINT valid_role CHECK (role IN ('CLIENT', 'PORTFOLIO_MANAGER', 'RESEARCHER', 'ADMIN')),
    CONSTRAINT valid_status CHECK (account_status IN ('ACTIVE', 'SUSPENDED', 'PENDING_VERIFICATION'))
);
...
-- Seed data - Demo users for testing (password is 'password' for all)
